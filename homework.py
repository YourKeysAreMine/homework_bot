...
def get_api_answer(timestamp: int):
    ...
    logger.debug('Начата отправка GET запроса к Яндекс.Домашка(тм)')
    try:
        ...
        logger.error('Ошибка при запросе к Яндекс.Домашка(тм)')
...
def main():
    ...
    if not check_tokens():
        # Если здесь убираю логгирование - валятся тесты :(
